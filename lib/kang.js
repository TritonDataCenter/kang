...
function knFetchAll(args, callback)
{
	...
	knFetchSnapshots(sources, function (err, snapshots) {
		if (err)
			...
			callback(null, knMergeSnapshots(snapshots));
	});
}
...
{
	knFetchMany(sources, knFetchSnapshot, callback);
}

function knFetchMany(sources, func, callback)
{
	var tasks, i;

	tasks = sources.map(
	    function (source) { return (func.bind(null, source)); });

	mod_nutil.runParallel(tasks, function (rv) {
		var err, snapshots, snapshot;
...
{
	knFetch(source, source['path'], callback);
}

function knFetch(source, path, callback)
{
...
function knFetchSchemaOne(source, callback)
{
	var path, p;
...
function knHandleRequest(svc, request, response, next)
{
	...
	if (request.path != svc.kns_uri_base + '/snapshot' &&
	    request.path != svc.kns_uri_base + '/schema') {
		response.send(404);
		next();
		return;
	}
